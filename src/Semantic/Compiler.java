...
import static Semantic.CommandType.*;
import static Semantic.VariableType.*;
import static Syntactic.NodeType.*;
...
public class Compiler {
    ...
    public List<Command> compile() {
        ...
        varTable = new VariableTable();
        exceptions.clear();
        addCommands(compileBlock(programNode, true, null));
        addCommand(new Command(EXIT, null, null, null));
        return result;
    }

    private List<Command> compileBlock(TreeNode blockNode, boolean program, List<Command> breakCommands) {
        ...
        for (TreeNode node : blockNode.getChildren()) {
            try {
                switch (node.getType()) {
                    case STMT_BLOCK:
                        addCommands(result, compileBlock(node, breakCommands));
                        break;
                    case STMT_IF:
                        addCommands(result, compileIf(node, breakCommands));
                        break;
                    case STMT_WHILE:
    ...
    private List<Command> compileBlock(TreeNode blockNode, List<Command> breakCommands) {
        return compileBlock(blockNode, false, breakCommands);
    }

    private List<Command> compileIf(TreeNode ifNode, List<Command> breakCommands) {
        List<Command> result = new ArrayList<>();
        ...
        addCommands(result, compileBlock(ifBlockNode, breakCommands));
        if (jmpToElse != null)
        ...
        if (elseBlockNode != null) {
            ...
            if (jmpToElse != null)
                jmpToElse.setAddress(result.size());
            addCommands(result, compileBlock(elseBlockNode, breakCommands));
    ...
    private List<Command> compileWhile(TreeNode whileNode) {
        ...
        addCommands(result, compileBlock(blockNode, breakCommands));
